...
(function(GlobalConstructor, global, modules, entry) {
  ...
  function RequireAsynchronously(id, path) {
    ...
    // Example:
    //   getHostFromCurrentUrl() → "http://localhost:8000"
    //   path → "components/counter.js"
    //   result → "http://localhost:8000/components/counter.js"
    var actualPath = getHostFromCurrentUrl() + "/" + path;
  ...
  function require(id) {
    ...
    if (asynchronously) {
      ...
      // Extract the actual file path portion after any namespace.
      var modulePath = moduleId.split("::")[1];

      // Create and initialize an asynchronous loader instance.
  ...
  typeof window !== "undefined" ? window : this,
  {
    ...
    "&::entry.js": [
      function(require, exports, module) {
        ...
        /**
         * Executes a sequence of verification tests to ensure that the runtime
         * module loader works correctly for both synchronous and asynchronous modules.
         *
         * Each test validates a different module type:
         *  - Synchronous modules (local static import)
         *  - Asynchronous remote modules (HTTP-based dynamic import)
         *  - Namespaced modules (custom identifier prefixes)
         *  - JSON, CSS, and JS module interoperability
         *
         * The test runner (`runTest`) compares expected and actual outputs for
         * correctness and logs results to the console.
         */
        async function runAllTests() {
          // -------------------------------------------------------------------------
          // 1. Test synchronous CommonJS-style module
          // -------------------------------------------------------------------------
          // This test verifies that the static module registry and `require()` function
          // properly return the default export from a locally registered module.
          runTest("Greeting Module Default Export", greetings, "Hello, World!");

          try {
            // -----------------------------------------------------------------------
            // 2. Test RPC module with dynamic remote loading
            // -----------------------------------------------------------------------
            // Uses <HTTP> marker to indicate that this module should be loaded
            // asynchronously from a remote endpoint. The default namespace symbol "&"
            // is used since no explicit namespace is defined.
            var rpc = await require("./dynamic/rpc.js/<HTTP>/");
            runTest("RPC Module getMessage Output", rpc.getMessage(), "Hello, World!");

            // -----------------------------------------------------------------------
            // 3. Test dynamic JSON module (no explicit namespace)
            // -----------------------------------------------------------------------
            // Verifies that JSON data can be fetched as a module and parsed into an
            // exportable JavaScript object through the runtime's async loader.
            var colors = await require("./dynamic/colors.json/<HTTP>");
            ...
            // -----------------------------------------------------------------------
            // 4. Test dynamic CSS module with explicit namespace (DynamicCSS)
            // -----------------------------------------------------------------------
            // Confirms that remote CSS files can be loaded either as raw text or as
            // `CSSStyleSheet` objects (depending on browser capability). The namespace
            // allows multiple CSS modules to coexist without conflict.
            var styles = await require("./dynamic/styles.css/<HTTP>/DynamicCSS");
            runTest(
            ...
            // -----------------------------------------------------------------------
            // 5. Test fully-qualified Microfrontend resource with namespace (Somewhere)
            // -----------------------------------------------------------------------
            // Demonstrates loading a CSS resource hosted on an external domain.
            // The module name begins with "https://" and ends with a namespace
            // declaration `/Somewhere`, which isolates it from other CSS modules.
            var somewhere = await require("https://www.microfrontends.com/resources/somewhere.css/<HTTP>/Somewhere");
            runTest(
              ...
              `),
              true // Enable lenient mode to ignore whitespace differences in CSS comparison
            );
            ...
            // -----------------------------------------------------------------------
            // Any failure in fetching, parsing, or executing a module is caught here.
            // This ensures the test suite continues even if one module fails.
      ...
      },
      {
        ...
        "./dynamic/styles.css/<HTTP>/DynamicCSS":"DynamicCSS::dynamic/styles.css/<HTTP>",
        "https://www.microfrontends.com/resources/somewhere.css/<HTTP>/Somewhere": "Somewhere::resources/somewhere.css/<HTTP>"
      }
    ],
    // Greetings module
    "&::greetings.js": [
