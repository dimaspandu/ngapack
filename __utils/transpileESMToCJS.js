...
export default function transpileESMToCJS(esmCode) {
  let code = removeWithSyntax(esmCode);
...
/**
 * Remove ES module import assertions (the "with { ... }" part)
 * from both static and dynamic imports.
 *
 * Examples:
 *   import colors from "./colors.json" with { type: "json" };
 *   => import colors from "./colors.json";
 *
 *   await import("file.css", { with: { type: "css" } });
 *   => await import("file.css");
 *
 * @param {string} code - The source code to process
 * @returns {string} - Cleaned code without "with { ... }"
 */
function removeWithSyntax(code) {
  return code
    // Remove "with { ... }" from static imports
    .replace(/\s+with\s*\{[^}]*\}/g, "")
    // Remove ", { with: { ... } }" from dynamic imports
    .replace(/,\s*\{\s*with\s*:\s*\{[^}]*\}\s*\}/g, "");
}
